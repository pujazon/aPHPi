...
<script>
	...
	//Incrementa el contador asociado al nombre pasado como parámetro 
	//Del included counters.php
	
	function incrCount(name){
		...
		<?php
			...
			$tmp2=$GLOBALS["id2"];
		?>
			console.log("ID1:<? $GLOBALS["id1"] ?> \n ID2: <? $GLOBALS["id2"] ?>"); // Una manzana verde

	}
	
	//Esta función redirige a la página pasada como parámetro.
	function Link(url){
		
	}	
	
	
	//Esta función es el handler que se encargará de 
	//Incrementar el contador asociado al nombre pasado como parámetro
	//Y redireccionar al enlace pasado como segundo parámetro
	
	function count_and_link(name,url,debug){
		if(debug){
			console.log("count&link(name,url){");
			console.log(name);
			console.log(url);
		}		
		incrCount(name);
